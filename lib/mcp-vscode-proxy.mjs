...
export class McpVsCodeContextProxy {
  ...
  constructor({
    ...
    // Optional: only set when a Windows App view exists (win32). The model cannot know the Windows
    // view's instance id, so the proxy injects it for the self-contained windows_app_* tools.
    this.windowsInstanceId = windowsInstanceId;
  ...
  async clientMessage(message) {
    ...
    // Windows MCP tools are self-contained. The proxy injects the Windows view's identity (when a
    // Windows view exists) and returns immediately, so a windows_app_* call can never fall through
    // to the Mobile device selection / follow-up / automation logic below.
    if (name.startsWith("windows_app_")) {
      if (this.windowsInstanceId) {
        args.sessionId = this.sessionId;
        args.instanceId = this.windowsInstanceId;
      }
      return message;
    }
  ...
  async serverMessage(message) {
    ...
    const relabelWindows = Boolean(this.windowsInstanceId);
    for (const tool of message.result.tools) {
      // Windows tools take sessionId/instanceId too, but the Windows view provides both, so they are
      // stripped from `required` and relabelled to match the Mobile behaviour.
      if (
        relabelWindows
        && typeof tool?.name === "string"
        && tool.name.startsWith("windows_app_")
        && tool.inputSchema
      ) {
        ...
        for (const property of ["sessionId", "instanceId"]) {
          if (tool.inputSchema.properties?.[property]) {
            tool.inputSchema.properties[property].description =
              "Provided automatically by the Windows App VS Code view.";
          }
        }
        continue;
      }
