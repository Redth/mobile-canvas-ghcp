...
import { validateWindowsAppHelperEntry } from "./windows-app-helper.mjs";
...
export function validateRuntimePreflight(key, entry) {
  if (!entry?.executable || !entry?.files?.[entry.executable]) {
    throw new Error(`${key} has no checksummed primary executable`);
  }
  return validateWindowsAppHelperEntry(key, entry);
}

export function resolveRuntimeCompanion(command, name) {
  const companion = join(dirname(command), name);
  if (!existsSync(companion)) {
    throw new Error(`runtime companion is missing next to ${command}: ${name}`);
  }
  return companion;
}
...
export async function materializeRuntime(manifest, entry, options = {}) {
  ...
  validateRuntimePreflight(key, entry);
  if (typeof entry.id !== "string" || entry.id.length < 12) {
    throw new Error(`${key} has an invalid runtime cache id`);
  }
  ...
  if (expectedFiles.every((name) => existsSync(join(target, name)))) return resolved;
...
export async function resolveCommand() {
  ...
  if (entry) validateRuntimePreflight(platformKey(), entry);
